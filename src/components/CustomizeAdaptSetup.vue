<template>
    ...
    <SectionExcerpt>
        Customize the conversation prompt template, or upload a custom JSON file
        containing a list of new agents. The default agent list can be
        downloaded on the More Information page and suits as a template for
        customization.
    </SectionExcerpt>

    <div>
        <Caption class="block ml-1 mb-2"
            >Change Prompt (must contain <code>{history}</code> and
            <code>{thread}</code>):</Caption
        >
        <Floater>
            <Textarea
                class="w-full"
                ...
                @input="
                    (event) =>
                        getConfigStore().replacePrompt(event.target.value)
                "
            ></Textarea>
        </Floater>
    </div>
    ...
    <div>
        ...
        <Floater>
            <div class="flex flex-row gap-3">
                ...
                />
                <Button :disabled="!hasNewAgents" @click="replaceAgents()">
